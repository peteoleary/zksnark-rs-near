# zkSNARK Contracts for NEAR

Setup instructions borrowed and adapted from https://github.com/near-examples/rust-status-message/blob/master/README.md
...
```bash
near dev-deploy --wasmFile res/setup.wasm --helperUrl https://near-contract-helper.onrender.com
```
...
The next command will call the contract's `new` method to set up the contract:

```bash
near call $CONTRACT_NAME from_zk '{"code": "aloha!"}' --accountId $CONTRACT_NAME
```
